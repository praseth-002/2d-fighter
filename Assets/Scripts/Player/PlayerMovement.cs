...
public class PlayerMovement : MonoBehaviour
{
    ...
    private void Update()
    {
        if (currentState == PlayerState.Hit || currentState == PlayerState.Dead) return;

        // Handle attacking freeze
        if (isAttacking)
        {
            animator.SetBool("IsRunning", false);
            attackTime -= Time.deltaTime;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        if (collision.collider.CompareTag("Ground"))
        {
            isGrounded = true;
            animator.SetBool("IsJumping", false);
            if (!isAttacking && currentState != PlayerState.Hit && currentState != PlayerState.Dead)
                currentState = PlayerState.Idle;
        }
    }
    ...
    public void EnterHitState(float duration)
    {
        ...
        StartCoroutine(HitStateCoroutine(duration));
    }

    private System.Collections.IEnumerator HitStateCoroutine(float duration)
    {
